...
{
  const children = childNodes.filter(filterElements)
...
{
  return childNodes.filter(filterElements).map(parseValue)
}
...
{
  return childNodes.filter(filterElements).map(parseParam)
}

function parseStruct(childNodes)
{
  return childNodes.filter(filterElements).reduce(parseMember, {})
}
